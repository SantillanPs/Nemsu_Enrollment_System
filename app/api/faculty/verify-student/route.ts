...
/**
 * API endpoint for faculty to directly verify student accounts
 * This allows faculty to bypass the document verification process
 */
export async function POST(request: Request) {
  try {
    ...
    if (!currentUser || !hasRoleAccess(currentUser.role, "FACULTY")) {
      return NextResponse.json(
        { error: "Only faculty members can verify student accounts" },
        { status: 403 }
      );
    }
    ...
    if (verificationStatus) {
      ...
      await prisma.document.updateMany({
        ...
        data: {
          status: "VERIFIED",
          verificationMessage:
            "Automatically verified because student account is verified by faculty",
        },
      });
    ...
    console.log(
      ...
      } by faculty ${currentUser.email} (${currentUser.id})`
    );
