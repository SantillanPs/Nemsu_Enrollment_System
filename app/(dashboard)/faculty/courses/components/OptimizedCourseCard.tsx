...
// Get enrollment status color - moved outside component to prevent recreation on each render
...
// Memoized Enrollments component
const SectionEnrollments = memo(({ enrollments }: { enrollments: Enrollment[] }) => {
  return (
              ...
              {enrollments.map((enrollment) => (
                <TableRow key={enrollment.id}>
                  ...
                  <TableCell className="text-right">
                    <Badge
                      className={getEnrollmentStatusColor(enrollment.status)}
                    >
                      {enrollment.status}
                    </Badge>
                  </TableCell>
...
SectionEnrollments.displayName = "SectionEnrollments";
  ...
  return (
    ...
    >
      ...
      {expandedSection === section.id && (
        <SectionEnrollments enrollments={section.enrollments} />
      )}
    </div>
  );
});
...
const CourseInfo = memo(({ course }: { course: Course }) => {
  return (
...
// Main CourseCard component wrapped in memo to prevent unnecessary re-renders
const CourseCard = memo(({ course }: CourseCardProps) => {
  ...
  const toggleSection = useCallback((sectionId: string) => {
    setExpandedSection(prevExpanded => prevExpanded === sectionId ? null : sectionId);
  }, []);

  // Memoize sections rendering to prevent unnecessary re-renders
  ...
  }, [course.sections, expandedSection, toggleSection]);

  // Memoize the course header to prevent unnecessary re-renders
  ...
  ), [course.code, course.name, course.status, course.description]);
