...
export default function FacultyDocuments() {
  ...
  const [filteredStudents, setFilteredStudents] = useState<StudentWithDocuments[]>([]);
  ...
  const mockDocuments: Document[] = [
    ...
    },
    {
      id: "3",
      type: "GRADES",
      fileUrl: "/documents/grades.pdf",
      status: "VERIFIED",
      verificationMessage: "All grades are in order.",
    ...
    },
    {
      ...
      status: "REJECTED",
      verificationMessage: "Document is incomplete. Please resubmit with all required signatures.",
      createdAt: "2023-08-09T11:45:00Z",
      profile: {
        id: "p2",
        firstName: "Emily",
        lastName: "Johnson",
    ...
    },
    {
      id: "5",
      type: "TOR",
      fileUrl: "/documents/transcript2.pdf",
      status: "PENDING",
      createdAt: "2023-08-08T16:30:00Z",
      profile: {
        id: "p3",
        firstName: "Michael",
        lastName: "Brown",
  ...
  const groupDocumentsByStudent = (docs: Document[]): StudentWithDocuments[] => {
    ...
    docs.forEach((doc) => {
      ...
      const allVerified = student.documents.every(d => d.status === "VERIFIED");
      const allRejected = student.documents.every(d => d.status === "REJECTED");
  ...
  useEffect(() => {
    // In a real app, this would fetch documents from your API
    // For now, we'll use the mock data
    setTimeout(() => {
      setDocuments(mockDocuments);
      const groupedStudents = groupDocumentsByStudent(mockDocuments);
            ...
            <SelectContent>
              {statusOptions.map((status) => (
                <SelectItem key={status} value={status}>
                  {status === "all" ? "All Statuses" : statusConfig[status].text}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
            ...
            {isLoading ? (
              <div className="flex justify-center items-center py-10">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-muted-foreground">Loading student documents...</p>
                </div>
              </div>
            ) : (
              <div className="grid gap-6">
                {filteredStudents
                  .filter((student) => student.status.toLowerCase() === tabValue.toLowerCase())
                  .map((student) => (
                    <Card key={student.profileId}>
                      <CardHeader className="pb-3">
                        <div className="flex justify-between items-start">
                          ...
                          <Badge className={statusConfig[student.status].color}>
                            {React.createElement(statusConfig[student.status].icon, { className: "h-3 w-3 mr-1" })}
                          ...
                          <AccordionItem value="documents">
                            <AccordionTrigger>
                              <div className="flex items-center">
                                ...
                                <span>Documents ({student.documents.length})</span>
                              </div>
                            </AccordionTrigger>
                            <AccordionContent>
                                ...
                                {student.documents.map((doc) => (
                                  <div key={doc.id} className="border rounded-md p-4">
                                    <div className="flex justify-between items-start mb-3">
                                      <div>
                                        <h4 className="font-medium">{documentTypeLabels[doc.type]}</h4>
                                        <p className="text-sm text-muted-foreground">
                                          Submitted: {new Date(doc.createdAt).toLocaleDateString()}
                                        </p>
                                      </div>
                                      <Badge className={statusConfig[doc.status].color}>
                                        {React.createElement(statusConfig[doc.status].icon, { className: "h-3 w-3 mr-1" })}
                      ...
                      <CardFooter>
                        <div className="w-full flex justify-between items-center">
                          ...
                          <div>
                            {student.isVerified ? (
                              <Badge className="bg-green-100 text-green-800">Account Verified</Badge>
                            ) : (
                ...
                {filteredStudents.filter((student) => student.status.toLowerCase() === tabValue.toLowerCase()).length === 0 && (
                  <div className="text-center py-10">
                    <h3 className="text-lg font-medium">No {tabValue} students</h3>
        ...
        <DialogContent>
            ...
            {selectedDocument && (
              <div className="space-y-2">
                <p className="text-sm font-medium">
                  Student: {selectedDocument.profile.firstName} {selectedDocument.profile.lastName}
                </p>
                <p className="text-sm">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={isSubmitting || (verificationStatus === "REJECTED" && !verificationMessage)}
              className={
                verificationStatus === "VERIFIED"
